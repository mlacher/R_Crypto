...
#-----------------------1.1 Read Database----------------------------#
Read_Share_DB<- function(Share_Type,Share_Name){
...
Plot_Share <- function (cShare_DB, begDate, endDate){
  Date_Add = paste(begDate,"/",endDate, sep = "")
  plot_cShare_DB <- cShare_DB[Date_Add]
  
  
  # par(mfrow=c(2,1))
  # a<-plot(plot_cShare_DB[,3])
  # b<-plot(plot_cShare_DB[,c(1,2)])
  return(plot_cShare_DB)
}
